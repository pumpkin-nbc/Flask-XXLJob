...
```text
dist/
  flask_xxljob-0.3.2-py3-none-any.whl
  flask_xxljob-0.3.2.tar.gz
```
...
.venv\Scripts\python.exe -m twine check dist\flask_xxljob-0.3.2-py3-none-any.whl dist\flask_xxljob-0.3.2.tar.gz
```
...
python -m pip install --index-url https://test.pypi.org/simple --no-deps flask-xxljob==0.3.2
python -c "import flask_xxljob; assert flask_xxljob.__version__ == '0.3.2'"
...
The tag starts the same `Release` workflow. It verifies that `v0.3.2`,
